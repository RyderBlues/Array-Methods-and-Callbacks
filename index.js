...
const final = fifaData.filter(function(element){
...
function getFinals(data) {
    return data.filter(function(item){return item.Stage === "Final"});
}
...
function getYears(data, getFinalsCB){
    return getFinalsCB(data).map(function(item){return item.Year});
}
...
// console.log('WINNERS TASK', getWinners(fifaData, getFinals));
...
function getWinnersByYear(data, getYearsCB, getWinnersCB) {
    ...
    return getYearsCB(data).map((item, index) => `In ${item}, ${getWinnersCB(data)[index]} won the world cup!`);
}
